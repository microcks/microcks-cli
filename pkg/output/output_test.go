...
func TestOperationLine(t *testing.T) {
	...
	for op, want := range cases {
		if got := openAPIOperationLine(spec, op); got != want {
			t.Errorf("openAPIOperationLine(%q) = %d, want %d", op, got, want)
		}
	}
	if got := openAPIOperationLine("/no/such/file.yaml", "GET /products"); got != 0 {
...
func TestGitHubActionsFileLineAnnotation(t *testing.T) {
	...
	plain, err := NewFormatter(FormatGitHubActions)
	if err != nil {
		t.Fatal(err)
	}
	out2, err := plain.Format(result)
	if err != nil {
		t.Fatal(err)
	}
