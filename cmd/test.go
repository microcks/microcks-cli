 ...
 func NewTestCommand(globalClientOpts *connectors.ClientOptions) *cobra.Command {
	 ...
	 var testCmd = &cobra.Command{
		 ...
		 Run: func(cmd *cobra.Command, args []string) {
			 ...
			 // Validate output-format flag value.
			 if outputFormat != "plain" && outputFormat != "github-actions" {
				 fmt.Println("--output-format must be one of: plain, github-actions")
				 os.Exit(1)
			 }
			 ...
			 // Print results in requested output format
			 if outputFormat == "github-actions" {
				 testResultDetails, err := mc.GetTestResultDetails(testResultID)
				 if err == nil {
					 for _, testCaseResult := range testResultDetails.TestCaseResults {
						 if testCaseResult.Success {
							 fmt.Printf("::notice title=Test Passed::Operation %s PASSED\n", testCaseResult.OperationName)
						 } else {
							 fmt.Printf("::error title=Test Failed::Operation %s FAILED\n", testCaseResult.OperationName)
						 }
					 }
				 }
			 }
	 ...
	 testCmd.Flags().StringVar(&outputFormat, "output-format", "plain", "Output format for test results. Supported: plain, github-actions")
